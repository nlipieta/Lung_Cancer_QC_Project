...
test = {}
for x in reference['entity:sample_id']:
    if reference.participant[reference[reference['entity:sample_id'] == x].index[0]] not in test:
        test[reference.participant[reference[reference['entity:sample_id'] == x].index[0]]] = 1
    else:
        test[reference.participant[reference[reference['entity:sample_id'] == x].index[0]]] += 1
...
#                total_count += 1
    ...
    for sample, value in edge_count.items():
        if reference.participant[reference[reference['entity:sample_id'] == sample].index[0]] == patient_name:
            if value == 0:
                test[patient_name] -= 1
    for key, value in edge_count.items():
        if reference.participant[reference[reference['entity:sample_id'] == key].index[0]] == patient_name:
            if value == (test[patient_name]-1):
            ...
            final_list.append([key, value, samples[patient_name], match_category])
...
final_df.to_csv('match_vs_sample_reference.tsv', sep = '\t', index = False)
